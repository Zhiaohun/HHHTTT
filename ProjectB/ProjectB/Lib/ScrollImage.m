...
- (instancetype)initWithCurrentController:(UIViewController *)viewcontroller urlString:(NSArray *)urls labString:(NSArray *)labs viewFrame:(CGRect)frame placeholderImage:(UIImage *)image startAnimated:(BOOL) isAnimated
{
    if (self = [super init]) {
        ...
        if (labs != nil) {
            _labArray = [NSMutableArray arrayWithArray:labs];
        }
       // _labArray = nil;
...
- (void)loadImageViews
{
    for (int i = 0; i < _imageArray.count+2; i ++) {
        ...
        //适应cell的宽高
            if (i == 0) {
                    imgView.frame = CGRectMake(0, 0, _imageWidth, _scrollview.frame.size.height-30);
                    _namelab.frame =CGRectMake(0, _frame.size.height-30, _imageWidth, 30);
                    _namelab.text =[_labArray objectAtIndex:_labArray.count-1];
            }else if (i == _imageArray.count+1){
                    imgView.frame = CGRectMake((_imageArray.count+1)*_imageWidth, 0, _imageWidth, _frame.size.height-30);
                    _namelab.frame = CGRectMake((_imageArray.count+1)*_imageWidth, _frame.size.height-30, _imageWidth, 30);
                    _namelab.text= [_labArray objectAtIndex:0];
            
            }else{
                    imgView.frame = CGRectMake(i*_imageWidth, 0, _imageWidth, _scrollview.frame.size.height-30);
                    _namelab.frame = CGRectMake(i*_imageWidth, _frame.size.height-30, _imageWidth, 30);
                    _namelab.text = [_labArray objectAtIndex:i-1];
            }
            [_scrollview addSubview:_namelab];
        }else{
                //根据图片大小自适应
            if (i == 0) {
...
- (void)scrollViewLoadImages
{
    for (int i = 0; i < _imageArray.count+2; i ++) {
        ...
        if (i == 0) {
            ...
            if (!_labArray) {
                labName= [_labArray objectAtIndex:0];
            }
            ...
            if (!_labArray) {
                labName = [_labArray objectAtIndex:i-1];
            }
