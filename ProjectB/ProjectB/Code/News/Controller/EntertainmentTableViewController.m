...
@interface EntertainmentTableViewController ()<KNBannerViewDelegate>
@property (nonatomic, assign) NSInteger countStart;
@property (nonatomic, strong) NewsEntertainmentBaseClass *entertainmentBaseModel;
...
-(void)dataRequest{
    NSString *URLStr = [NSString stringWithFormat:@"%@/%lu-20.html",URL_Amusement,self.countStart];
    ...
    [LLNetWorkingRequest reuqestWithType:GET Controller:self URLString:URLStr Parameter:nil Success:^(NSDictionary *dic) {
        ...
        NewsEntertainmentT1348648517839 *model = self.entertainmentBaseModel.t1348648517839[0];
        ...
        for (NewsEntertainmentT1348648517839 *model in self.entertainmentBaseModel.t1348648517839) {
...
- (void)bannerView:(KNBannerView *)bannerView collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSInteger)index{
    NewsEntertainmentT1348648517839 *model = self.dataArray[0];
    NewsWebViewController *webVC = [[NewsWebViewController alloc] init];
    if (index == 0) {
       // webVC.URLHtml = model.url;
    }else{
